...
interface ProjectCardExcelsiorProps {
  project: ProjectItem;
  onOpenProject: (slug: string) => void;
}

export const ProjectCardExcelsior: React.FC<ProjectCardExcelsiorProps> = ({
  project,
  onOpenProject,
}) => {
  const excelAsset01 = '/images/Mockup2.webp';
  const excelAsset02 = '/images/Etiquette Excel 4.webp';
  ...
  return (
    <section className="py-20 md:py-28 px-6 sm:px-10 md:px-16 max-w-[1360px] mx-auto w-full">
      <div className="pt-10 border-t border-[#D9D7D1]">
        ...
        <div className="mb-10 pb-6 border-b border-[#D9D7D1]">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div className="flex items-center gap-4 font-mono-custom text-[11px] uppercase">
              <span className="text-[#0A0A0A]/60 hidden sm:inline-block">BELGIAN BEER BRANDING</span>
              <button
                onClick={() => onOpenProject(project.slug)}
                data-cursor="project"
                data-cursor-text="VIEW EXCELSIOR →"
        ...
        <div className="editorial-hero-frame mb-12">
          ...
          >
            <div className="relative aspect-[16/10] md:aspect-[16/9] w-full overflow-hidden">
              <img
                src={excelAsset01}
                alt="Excelsior Belgian Sparkling Wine Bottle"
              ...
              <div className="absolute top-4 left-4 z-10 flex items-center gap-2">
                <span className="bg-[#0A0A0A] text-[#F3F2EE] font-mono-custom text-[10px] md:text-[11px] px-3 py-1 uppercase tracking-widest">
                  MASTER BOTTLE PACKAGING — METHODE TRADITIONNELLE
                </span>
              </div>
          ...
          <div className="mt-3 flex justify-between items-center font-mono-custom text-[10px] uppercase text-[#0A0A0A]/50">
            <span>FIG. 01 — LUXURY 75CL BOTTLE PACKAGING / GOLD FOIL EMBOSSING</span>
            <span>BELGIUM</span>
          </div>
        </div>
        ...
        {/* 4. SECONDARY VISUAL (Curated Label Flat Presentation) */}
        <div className="editorial-landscape-frame">
          <div
            data-cursor="project"
            onClick={() => onOpenProject(project.slug)}
            className="group cursor-pointer overflow-hidden border border-[#D9D7D1] bg-[#FAF8F5] p-6 sm:p-10 flex items-center justify-center"
          >
            <div className="relative max-w-lg w-full overflow-hidden">
              <img
                src={excelAsset02}
                alt="Excelsior Label Flat Architecture"
          ...
          <div className="mt-3 flex justify-between items-center font-mono-custom text-[10px] uppercase text-[#0A0A0A]/50">
            <span className="font-semibold text-[#0A0A0A]">02 / LABEL ARCHITECTURE</span>
            <span>CUSTOM DIECUT &amp; EMBOSSED HOT-FOIL MATRIX</span>
          </div>
        </div>
      </div>
    </section>
