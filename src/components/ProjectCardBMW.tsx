...
interface ProjectCardBMWProps {
  project: ProjectItem;
  onOpenProject: (slug: string) => void;
}

export const ProjectCardBMW: React.FC<ProjectCardBMWProps> = ({ project, onOpenProject }) => {
  return (
    <section id="hero-project-section" className="py-20 md:py-28 px-6 sm:px-10 md:px-16 max-w-[1360px] mx-auto w-full">
      <div className="pt-10 border-t border-[#D9D7D1]">
        ...
        <div className="mb-10 pb-6 border-b border-[#D9D7D1]">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div className="flex items-center gap-4 font-mono-custom text-[11px] uppercase">
              <span className="text-[#0A0A0A]/60 hidden sm:inline-block">BMW BELUX × RBFA</span>
              <button
                onClick={() => onOpenProject(project.slug)}
                data-cursor="project"
                data-cursor-text="VIEW BMW →"
        ...
        {/* 2. MAIN HERO VISUAL (Controlled Editorial Landscape) */}
        <div className="editorial-hero-frame mb-12">
          <div
            data-cursor="project"
            data-cursor-text="VIEW PROJECT →"
            onClick={() => onOpenProject(project.slug)}
            className="group relative cursor-pointer block overflow-hidden bg-[#0A0E17] border border-[#D9D7D1] shadow-sm"
          >
            <div className="relative aspect-[16/9] w-full overflow-hidden">
              ...
              {/* Editorial Badge */}
              <div className="absolute top-4 left-4 font-mono-custom text-[10px] uppercase tracking-widest bg-[#0A0A0A] text-white px-2.5 py-1">
                MASTER KEY VISUAL — ELECTRIC DYNAMICS
              </div>
          ...
          <div className="mt-3 flex justify-between items-center font-mono-custom text-[10px] uppercase text-[#0A0A0A]/50">
            <span>FIG. 01 — MASTER CAMPAIGN KEY VISUAL (1920×1080PX)</span>
            <span>BMW BELGIUM / RBFA</span>
          </div>
        </div>
        ...
        {/* 4. SECONDARY VISUALS (Curated 2-Column Square Grid) */}
        <div className="editorial-grid-2">
          {/* Secondary 01: Doku x iX1 */}
          <div
            data-cursor="project"
            onClick={() => onOpenProject(project.slug)}
            className="group cursor-pointer overflow-hidden border border-[#D9D7D1] bg-[#0A0E17]"
          >
            <div className="relative aspect-square w-full overflow-hidden">
              <img
                src={project.secondaryImages?.[0] || project.heroImage}
                alt="BMW Red Devils Player × Car Visual Design"
            ...
            <div className="p-3 bg-white/90 border-t border-[#D9D7D1] flex justify-between items-center font-mono-custom text-[10px] uppercase text-[#0A0A0A]">
              <span className="font-semibold">01 / PLAYER × CAR SERIES</span>
              <span className="text-[#0A0A0A]/50">JÉRÉMY DOKU × BMW iX1</span>
            </div>
          </div>

          {/* Secondary 02: Campaign Overview */}
          <div
            data-cursor="project"
            onClick={() => onOpenProject(project.slug)}
            className="group cursor-pointer overflow-hidden border border-[#D9D7D1] bg-[#0A0E17]"
          >
            <div className="relative aspect-square w-full overflow-hidden">
              <img
                src={project.secondaryImages?.[1] || project.heroImage}
                alt="BMW Red Devils Campaign Overview"
                className="w-full h-full object-contain transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-[1.02]"
                loading="lazy"
                decoding="async"
              />
            ...
            <div className="p-3 bg-white/90 border-t border-[#D9D7D1] flex justify-between items-center font-mono-custom text-[10px] uppercase text-[#0A0A0A]">
              <span className="font-semibold">02 / MODULAR SYSTEM</span>
              <span className="text-[#0A0A0A]/50">17 CAMPAIGN DECLINATIONS</span>
            </div>
          </div>
        </div>
      </div>
